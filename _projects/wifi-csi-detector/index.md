...
description: "A digital signal processing system designed to simulate wireless Channel State Information (CSI) amplitude tracking, filter high-frequency network noise, and extract variance features for automated human presence detection."
skills: 
  - MATLAB
  ...
  - Algorithm Design
main-image: "csi-thumbnail.jpg"
---

## Project Overview
This project targets the development of an embedded-ready signal processing pipeline that leverages wireless **Channel State Information (CSI)** to detect human movement without cameras. By simulating a raw, noisy $18\text{ dB}$ SNR WiFi amplitude stream, the system models physical environment changes (human intrusion), filters out high-frequency wireless noise using a light recursion technique, and applies statistical windowing to make binary security decisions.

### Key Engineering Features
* **Zero-Toolbox Dependency:** The algorithm is constructed entirely using pure mathematical formulas and base MATLAB matrix operations, ensuring effortless porting to production environments.
* **Embedded-Ready Low-Pass Filter:** Implements an Exponential Moving Average (EMA) topology ($a = 0.05$). This acts as a first-order recursive digital low-pass filter, requiring minimal memory overhead and no complex matrix math, making it perfect for microcontrollers.
* **Automated Decision Engine:** Computes a localized, moving standard deviation across a sliding $200\text{ ms}$ matrix window. If the variance spikes past an empirical $1.0\text{ dB}$ threshold, a binary classification engine trips a `[MOTION DETECTED]` state.

---

## Complete MATLAB Pipeline Simulator
...
%              presence. Zero toolboxes required.
...
figure('Name', 'Origin AI Sensing Simulation Pipeline', 'NumberTitle', 'off');
...
fprintf('====================================================\n');
fprintf('     ORIGIN AI WORKFLOW PROCESSING COMPLIANCE       \n');
fprintf('====================================================\n');
